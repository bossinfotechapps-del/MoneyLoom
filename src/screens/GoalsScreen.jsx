...
export default function GoalsScreen({ bottomSpace = 100, onToast }) {
  ...
  return <View style={{ flex: 1 }}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: bottomSpace, gap: 14 }}>
      <ScreenHeading title="Savings goals" subtitle="Build towards what matters to you" eyebrow="YOUR PLANS" right={<Action label="+ New" onPress={() => openEditor(null)} />} />
...
const s = StyleSheet.create({ heading: { flexDirection: 'row', alignItems: 'center', gap: 8 }, card: { padding: 18, borderRadius: 20, backgroundColor: C.surface, gap: 14, borderWidth: 1, borderColor: C.line, elevation: 1 }, amount: { fontSize: 26, fontWeight: '800', color: C.invest, letterSpacing: -0.5 }, track: { height: 8, backgroundColor: C.lineSoft, borderRadius: 5, overflow: 'hidden' }, fill: { height: 8, backgroundColor: C.invest, borderRadius: 5 }, button: { backgroundColor: C.chip, borderRadius: 13, paddingVertical: 12, paddingHorizontal: 14, alignItems: 'center', alignSelf: 'flex-start' }, buttonText: { color: C.investDark, fontWeight: '800' }, danger: { backgroundColor: '#FCEBEB' }, input: { backgroundColor: C.surface, borderWidth: 1, borderColor: C.line, borderRadius: 13, color: C.ink, fontSize: 16, paddingHorizontal: 12, paddingVertical: 12 }, row: { flexDirection: 'row', gap: 10, flexWrap: 'wrap' }, history: { flexDirection: 'row', gap: 8, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: C.line } });
